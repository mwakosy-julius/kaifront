...
const DetailsTable: React.FC<DetailsTableProps> = ({ details }) => {
  ...
  return (
    <div className="frosted-glass p-6 fade-in">
      <h2 className="text-2xl font-semibold mb-4">Top Taxa Details</h2>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-gray-600">
              ...
              >
                Genus {sortField === 'genus' && (sortOrder === 'asc' ? '↑' : '↓')}
              </th>
              <th
                className="p-3 cursor-pointer hover:text-cyan-400"
                onClick={() => handleSort('phylum')}
              >
                Phylum {sortField === 'phylum' && (sortOrder === 'asc' ? '↑' : '↓')}
              </th>
              <th
                className="p-3 cursor-pointer hover:text-cyan-400"
                onClick={() => handleSort('kmer_count')}
              >
                K-mer Hits {sortField === 'kmer_count' && (sortOrder === 'asc' ? '↑' : '↓')}
              </th>
              <th
                className="p-3 cursor-pointer hover:text-cyan-400"
                onClick={() => handleSort('distance')}
              >
                Avg Mismatch {sortField === 'distance' && (sortOrder === 'asc' ? '↑' : '↓')}
              </th>
              <th
                className="p-3 cursor-pointer hover:text-cyan-400"
          ...
          <tbody>
            {sortedDetails.map((row, idx) => (
              <tr key={idx} className="border-b border-gray-700 hover:bg-gray-700/50">
                ...
                <td className="p-3">{row.distance ? row.distance.toFixed(2) : 'N/A'}</td>
