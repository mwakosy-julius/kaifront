...
const handleError = (error: AxiosError<{ message?: string; error?: string; code?: string }>): ApiError => {
    const apiError = {
        ...
        if (error.response?.status === 401 && originalRequest) {
            try {
                ...
                if (refreshToken) {
                    ...
                    originalRequest.headers.Authorization = `Bearer ${accessToken || Cookies.get(ACCESS_TOKEN_COOKIE)}`;
